...
- (instancetype)initWithType:(JYIdentifyType)type
{
    ...
    if (self) {
        ...
        if (rType != JYIdentifyTypeNameOnly) {
        ...
        if (rType == JYIdentifyTypeName) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                imgView.rImageView.userInteractionEnabled = NO  ;
                
                imgView.tag = 1000 + i ;

                @weakify(self)
                [[imgView.rBgView rac_signalForControlEvents:UIControlEventTouchUpInside]subscribeNext:^(UIButton *x) {
                    @strongify(self)
                    if (self.rImageArray) {
                        self.rAddImageBlock(imgView) ;
                    }
            ...
            for (int i = 0; i< 3 ; i++) {
                UILabel *labe= [self jyCreateLabelWithTitle:titles[i] font:13 color:kTextBlackColor align:NSTextAlignmentCenter] ;
                [self addSubview:labe];
                [rTitlesArray addObject:labe];
            }
        }else if (rType == JYIdentifyTypeNameOnly) {
            ...
            imgLabel0.rImageView.image = [UIImage imageNamed:@"ident_blue"] ;
            imgLabel0.rHighlighted = YES ;
            JYImageLabe *imgLabel1 = self.rProgressImgArr[1] ;
            imgLabel1.rImageView.image = [UIImage imageNamed:@"ident_blue"] ;
...
-(void)layoutSubviews {
    ...
    [self.rProgressImgArr mas_distributeViewsAlongAxis:MASAxisTypeHorizontal withFixedItemLength:25 leadSpacing:37.5 tailSpacing:37.5] ;
    ...
    if (rType != JYIdentifyTypeName && rType!= JYIdentifyTypeNameOnly) {
        return ;
    }
...
-(UILabel*)rBottomLabel {
    ...
    if (_rBottomLabel == nil) {
        _rBottomLabel = [self jyCreateLabelWithTitle:@"添加借记卡（储蓄卡）" font:14 color:kTextBlackColor align:NSTextAlignmentLeft] ;
    }
...
-(UILabel*)rLabel {
    ...
    if (_rLabel == nil) {
        _rLabel = [self jyCreateLabelWithTitle:@"" font:12 color:kTextBlackColor align:NSTextAlignmentCenter] ;
    }
...
-(UIImageView*)rImageView {
    ...
    if (_rImageView == nil) {
        ...
        _rImageView.image = [UIImage imageNamed:@"ident_normal"] ;
