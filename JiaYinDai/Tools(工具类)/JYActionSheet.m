...
@interface JYActionSheet ()<UIGestureRecognizerDelegate>
{
    UIWindow *window;
}
...
-(void)loadUI{
    ...
    [Cancebtn setExclusiveTouch:YES];
    [Cancebtn setFrame:CGRectMake(0, CGRectGetHeight(ButtomView.bounds) - ItemHeight, W, ItemHeight)];
    [Cancebtn setTitleColor:kBlackColor forState:UIControlStateNormal];
    ...
    for (NSString *Title in _Titles) {
        ...
        //        [btn setImage:[UIImage imageNamed:[_arrayImages objectAtIndex:index]] forState:UIControlStateNormal];
        //        [btn setTitleEdgeInsets:UIEdgeInsetsMake(40, 0, 40, 0)];
         ...
         usingSpringWithDamping:0.8f
          initialSpringVelocity:0
                        options:UIViewAnimationOptionAllowUserInteraction animations:^{
                            // 设置背景透明
                            [weak setBackgroundColor:[UIColor colorWithRed:0.40  green:0.40  blue:0.40 alpha:0.5]];
         ...
         usingSpringWithDamping:0.8f
          initialSpringVelocity:0
                        options:UIViewAnimationOptionShowHideTransitionViews animations:^{
                            // 将options 设置成 UIViewAnimationOptionShowHideTransitionViews 解决dissmiss过程中的卡顿
                            // 直接设置成透明，“屏蔽”掉下方按钮应经消失但整个控件仍然遮蔽屏幕的效果
                            [weak setBackgroundColor:[UIColor clearColor]];
                            [TopView setFrame:CGRectMake(0, 0, W, H)];
